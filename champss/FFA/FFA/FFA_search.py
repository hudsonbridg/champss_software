...
from .single_pointing_FFA import detections_dtype
...
def remove_baseline(pgram, b0=50, bmax=1000):
    ...
    for iw, width in enumerate(pgram.widths):
        ...
        for i in range(len(sections) - 1):
            s[sections[i] : sections[i + 1]] /= np.median(
                s[sections[i] : sections[i + 1]]
            )
        pgram.snrs[:, iw] = s
...
def barycentric_shift(pgram, shifted_freqs, shift_min, shift_max):
    ...
    """
    freqs_subset = pgram.freqs[
        shift_min : shift_max + 1
    ]  # Extract frequency range once
    ...
    for iw in range(len(pgram.widths)):
        ...
        f = interpolate.interp1d(
            shifted_freqs, s, kind="nearest", bounds_error=False, fill_value=0
        )
...
def rogue_width_filter(peaks, snrs, widths):
    ...
    filtered_peaks = [
        ...
        if not (
            ...
            and snrs[peak.ip][peak.iw]
            > 2 * (sum(snrs[peak.ip]) - snrs[peak.ip][peak.iw])
        )
    ]
        ...
        for birdie in birdies:
            for harmonic_order in range(1, num_birdie_harmonics + 1):
                ...
                if abs(peak.period - harmonic_period) < birdie_tolerance:
                    ...
                    peak_width = 0
                    while (
                        pgram.snrs[max(0, period_index - peak_width)][peak.iw]
                        > min_rfi_sigma
                    ) and (
                        pgram.snrs[min(pgram_len - 1, period_index + peak_width)][
                            peak.iw
                        ]
                        > min_rfi_sigma
                    ):
                        peak_width += 1
                    for index in range(
                        max(0, period_index - peak_width),
                        min(pgram_len, period_index + peak_width),
                    ):
                        pass
...
def rate_periodogram(peaks, dm, birdies):
    ...
    for harmonic_order in range(2, 10):
        ...
        harmonic_peaks = list(
            filter(
                lambda peak: abs(peak.period - harmonic_period) / harmonic_period
                < tolerance,
                peaks,
            )
        )
...
def FFA_search(
    ...
    barycentric_beta = barycenter.get_barycentric_correction(
        str(ra), str(dec), mjd_time, tobs
    )
    ...
    for k, bird in known_birdies["known_bad_frequencies"].items():
        if barycentric_beta != 0:
            birdies.append(
                1 / barycenter.bary_from_topo_freq(bird["frequency"], barycentric_beta)
            )
        else:
    ...
    ts_0, pgram_0, peaks_0 = periodogram_form_dm_0(
        ts[0], p_min, tobs / 8, min_rfi_sigma
    )
    for peak in peaks_0:
        if barycentric_beta != 0:
            birdies.append(
                1 / barycenter.bary_from_topo_freq(peak.freq, barycentric_beta)
            )
        else:
    ...
    ts_test, pgram_test, peaks_test = periodogram_form(
        ts[1],
        dms[1],
        birdies,
        shifted_freqs,
    ...
    if write_ffa_candidates or write_ffa_detections:
        detections = np.array(
            [
                (dms[dm_trial], 1 / peak.period, peak.snr, peak.width)
                for dm_trial in range(n_dm_trials)
                for peak in peaks_array[dm_trial]
            ],
            dtype=detections_dtype,
        )
        ...
        if len(detections) > 0:
            ...
            print(
                f"cluster_dm_spacing: {cluster_dm_spacing}, cluster_df_spacing: {cluster_df_spacing}"
            )
            ...
            (
                ...
                detections, cluster_dm_spacing, cluster_df_spacing, plot_fname=""
            )
            ...
            for cluster in clusters.values():
                ...
                max_p_index = np.abs(
                    pgram_0.periods - 1 / (min(cluster.unique_freqs) - 0.001)
                ).argmin()
                min_p_index = np.abs(
                    pgram_0.periods - 1 / (max(cluster.unique_freqs) + 0.001)
                ).argmin()
                freq_range = np.array(
                    [
                        1 / pgram_0.periods[period_index]
                ...
                dm_freq_widths = np.array(
                    [
                        [
                            pgram_0.widths[
                                np.argmax(pgram_array[dm_index].snrs[period_index])
                            ]
                            for period_index in range(max_p_index, min_p_index, -1)
                        ]
                        for dm_index in range(min_dm_index, max_dm_index)
                ...
                if cluster.sigma > 10:
                    ...
                    os.makedirs(
                        f"{basepath}/{date_string}/{np.round(ra, 2)}_{np.round(dec, 2)}/plots/",
                        exist_ok=True,
                    )
                    ...
                    fig.tight_layout()
                    plt.savefig(
                        plot_path, bbox_inches=None, format="png", transparent=False
                    )
                    plt.close(fig)
                else:
                    plot_path = ""
    ...
    if write_ffa_detections and len(detections) > 0:
        ...
        detection_directory_name = os.path.join(
            directory_name, f"detections_FFA/{date.year}/{date.month}/{date.day}"
        )
        os.makedirs(detection_directory_name, exist_ok=True)
        file_path = os.path.join(
            detection_directory_name,
            f"{np.round(ra, 2)}_{np.round(dec, 2)}_FFA_detections.npz",
        )
    ...
    if write_ffa_candidates and len(candidates) > 0:
        ...
        candidate_directory_name = os.path.join(
            directory_name, f"{date_string}/{np.round(ra, 2)}_{np.round(dec, 2)}"
        )
        os.makedirs(candidate_directory_name, exist_ok=True)
        file_path = os.path.join(
            candidate_directory_name,
            f"{np.round(ra, 2)}_{np.round(dec, 2)}_FFA_candidates.npz",
        )
    ...
    if create_periodogram_plots:
        plot_directory_name = os.path.join(
            directory_name, f"{date_string}/{np.round(ra, 2)}_{np.round(dec, 2)}/plots"
        )
        os.makedirs(plot_directory_name, exist_ok=True)
        file_path = os.path.join(
            plot_directory_name,
            f"FFA_periodogram_ra_{np.round(ra, 2)}_dec_{np.round(dec, 2)}_snr_{np.round(best_peak.snr, 2)}_dm_{np.round(best_dm, 3)}.png",
        )

        fig, ax = plt.subplots(figsize=(15, 5))
        ax.plot(
            best_pgram.periods,
            best_pgram.snrs.max(axis=1),
            linewidth=0.3,
            marker="o",
            markersize=0.7,
        )
    ...
    if create_profile_plots:
        ...
        file_path = os.path.join(
            plot_directory_name,
            f"FFA_profile_ra_{np.round(ra, 2)}_dec_{np.round(dec, 2)}_snr_{np.round(best_peak.snr, 2)}_p_{np.round(best_peak.period, 6)}_dm_{np.round(best_dm, 3)}.png",
        )
        ...
        plt.subplot(211)
        plt.title(
            f"Folded profile at p={np.round(best_peak.period, 4)}s, dm={np.round(best_dm, 3)}, {date_string}"
        )
    ...
    if write_ffa_stack:
        ...
        if os.path.exists(file_path):
            with h5py.File(file_path, "a") as hf:
                if np.array_equal(dms, hf["dms"]) and np.array_equal(
                    pgram_array[0].periods, hf["periods"]
                ):
                    ...
                    for dm_index in range(n_dm_trials):
                        ...
                        stack_snrs = stack_snrs + (
                            pgram_array[dm_index].snrs / stack_length
                        )
                    ...
                    log.error(
                        "Cannot add data to stack as the DM range or period range is different!"
                    )
            ...
            with h5py.File(file_path, "w") as hf:
                hf.create_dataset(
                    "snrs",
                    data=[
                        pgram_array[dm_index].snrs for dm_index in range(n_dm_trials)
                    ],
                )
    ...
    if len(candidates) > 0:
        ...
        log.info(
            f"Best candidate: F0: {best_candidate['freq']:.3f} DM: {best_candidate['dm']:.2f} Sigma: {best_candidate['sigma']:.2f}"
        )
