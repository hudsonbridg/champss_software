#!/usr/bin/env python
# Copied from CHIMEFRB/ch_frb_l1_utils with addition the spulsar writer parameters
# fmt: off
"""
...
class RpcClient:
    ...
    def get_bonsai_variances(self, beams=[], servers=None, wait=True, timeout=-1):
        if servers is None:
            servers = self.servers.keys()
        tokens = []
        for k in servers:
    ...
    def stream(
        ...
        for k in servers:
            ...
            args = msgpack.packb([acq_name, acq_dev, acq_meta, acq_beams, new_stream, max_chunks])
            tokens.append(self.token)
            self.sockets[k].send(req + args)
        if not wait:
            return tokens
...
def main():
    ...
    parser.add_argument("--stop-stream", help="Stop Streaming files to the archiver")
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Set a limit on the number of (1-second) chunks to stream to disk.'
    )
    parser.add_argument(
        "--rate",
        action="store_true",
    ...
    if opt.max_fpga_plot:
        ...
        while True:
            ...
            if t1 - t0 > opt.max_fpga_plot:
                break
            fpgas = client.get_max_fpga_counts(wait=True, timeout=3.0)
            for f, server in zip(fpgas, servers.values()):
                print("", server)
                if f is None:
                    print("  None")
    ...
    if opt.stop_stream:
        ...
        for b in opt.stream_beams:
            ...
            for w in words:
                beams.append(int(w))
        patterns = client.stream('', opt.stream_base, opt.stream_meta, beams)
        print("Streaming to:", patterns)
        doexit = True
